...
int main(int argc, char **argv) {
  if (argc < 4) {
    printf("Usage: ./bsort B <infile> <outfile>\n");
    exit(0);
  }
  ...
  int numcount = size/sizeof(int);
  printf("num count in file = %d\n",numcount);
  int array[numcount];
  for (int i = 0; i < numcount; i++)
  {
        fread(array, sizeof(int), numcount,fin);
  }
  /*for (int i=0; i<numcount; i++)
  {
          printf("%d\n",array[i]);
  ...
  bucket_sort(array, numcount, B);
  fout = fopen(argv[3], "w");
  fwrite(array, sizeof(int), numcount, fout);
  fclose(fout);
#ifdef DEBUG
  printf("Result: ");
  print_array(array, numcount);
#endif
  verify_array(array, numcount);
}
